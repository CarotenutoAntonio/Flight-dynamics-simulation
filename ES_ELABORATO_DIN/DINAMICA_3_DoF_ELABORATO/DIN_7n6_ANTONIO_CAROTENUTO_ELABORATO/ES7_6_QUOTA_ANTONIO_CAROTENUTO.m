...
%%
%% modifico l'esercizio perchè considero diverse quote
%%
...
if (myAC.err == -1)
    ...
    % modifico qua
    NzEG=20;
    vzEG_0 = linspace(0,10000,NzEG); %Velocità di volo [m/s]
...
valpha0_deg=zeros(NzEG,1);
vdelta_e0_deg=zeros(NzEG,1);
vdelta_T0=zeros(NzEG,1);
vdelta_s0_deg=zeros(NzEG,1);
...
figure(1)
subplot(3,1,1);
plot(vzEG_0,valpha0_deg','b-o','LineWidth',1.5);
title('alfa');
ylabel('$\alpha_{0} (deg)$','Interpreter','latex');
xlabel('$h_{0}$','Interpreter','latex');

subplot(3,1,2);
plot(vzEG_0,vdelta_e0_deg','b-o','LineWidth',1.5);
...
subplot(3,1,3);
plot(vzEG_0,vdelta_T0','b-o','LineWidth',1.5);
...
xlabel('$h_{0}$','Interpreter','latex');
 ...
 %Results.vzEG_0 = v0;
    ...
    %save('linear_resultsANTONIOCAROTENUTO_quota.mat','Results')