...
if (myAC.err == -1)
    ...
    Vq0 = convangvel(linspace(-5,5,Nq),'deg/s','rad/s'); %Velocità angolare di beccheggio
...
valpha0_deg=zeros(Nq,1);
vdelta_e0_deg=zeros(Nq,1);
vdelta_T0=zeros(Nq,1);
vdelta_s0_deg=zeros(Nq,1);
...
figure(1)
subplot(3,1,1);
plot(Vq0,valpha0_deg','b-o','LineWidth',1.5);
title('alfa');
ylabel('$\alpha_{0} (deg)$','Interpreter','latex');
xlabel('$q_{0}$','Interpreter','latex');

subplot(3,1,2);
plot(Vq0,vdelta_e0_deg','b-o','LineWidth',1.5);
...
subplot(3,1,3);
plot(Vq0,vdelta_T0','b-o','LineWidth',1.5);
