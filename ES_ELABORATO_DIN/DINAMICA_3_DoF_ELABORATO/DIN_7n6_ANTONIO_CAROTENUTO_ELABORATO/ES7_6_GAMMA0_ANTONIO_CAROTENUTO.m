...
if (myAC.err == -1)
    ...
    Ngam=20;
    Vgamma0 = convang(linspace(-10,10,Ngam),'deg','rad'); %Velocità di volo [m/s]
...
valpha0_deg=zeros(Ngam,1);
vdelta_e0_deg=zeros(Ngam,1);
vdelta_T0=zeros(Ngam,1);
vdelta_s0_deg=zeros(Ngam,1);
...
figure(1)
subplot(3,1,1);
plot(Vgamma0,valpha0_deg','b-o','LineWidth',1.5);
title('alfa');
ylabel('$\alpha_{0} (deg)$','Interpreter','latex');
xlabel('$\gamma_{0}$','Interpreter','latex');

subplot(3,1,2);
plot(Vgamma0,vdelta_e0_deg','b-o','LineWidth',1.5);
...
subplot(3,1,3);
plot(Vgamma0,vdelta_T0','b-o','LineWidth',1.5);
